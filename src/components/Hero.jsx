...
import '../App.css';
import nurseryImage from '../assets/509iyt6wIO6l.jpg';
import playgroundImage from '../assets/xyPYstQkIAf5.jpg';

const Hero = () => {
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden pb-5">
          ...
          <div className="text-center lg:text-right space-y-8">
            <div className="space-y-4">
              <div className="flex items-center justify-center lg:justify-end space-x-2 rtl:space-x-reverse">
                ...
                <span className="text-[#023047] font-semibold text-lg">مرحباً بكم في</span>
              ...
              <h1 className="text-5xl lg:text-7xl font-bold leading-tight">
                ...
                <br />
                <span className="text-purple-800">براعم المستقبل</span>
              </h1>
              
              <div className="flex items-center justify-center lg:justify-start space-x-1 rtl:space-x-reverse">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className="w-6 h-6 text-yellow-400 fill-current " style={{animationDelay: `${i * 0.2}s`}} />
                ))}
              </div>
            </div>

           <p className="text-xl lg:text-2xl text-[#023047] leading-relaxed max-w-3xl mx-auto lg:mx-0">
  نوفر بيئة آمنة ومحفزة لنمو طفلك وتطوير مهاراته الاجتماعية والتعليمية، 
  حيث نهتم بكل جانب من جوانب نموه العقلي والعاطفي. 
  نعتمد على وسائل تعليمية حديثة وألعاب تفاعلية تدمج بين المرح والتعلم، 
  تحت إشراف فريق متخصص من المربيات المؤهلات اللاتي يقدمن الرعاية بكل حب واهتمام. 
  
</p>


            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-end">
              <button className="bg-[#023047] text-white px-8 py-4 rounded-full text-lg font-semibold hover:from-pink-600 hover:to-purple-700 transition-all duration-300 hover-lift shadow-xl flex items-center justify-center space-x-2 rtl:space-x-reverse">
                <span>احجز جولة مجانية</span>
                <ArrowLeft className="w-5 h-5 rotate-180" />
              </button>
              
              <button className="border-2 border-[#023047] text-[#023047] px-8 py-4 rounded-full text-lg font-semibold hover:bg-[#023047] hover:text-white  transition-all duration-300 hover-lift">
                تعرف على خدماتنا
              </button>
            </div>
